...
class YOLOLayer(nn.Module):
    ...
    def forward(self, xin, labels=None, shap=False):
        """
        In this
        Args:
        ...
        to_xshift = np.broadcast_to(
            np.arange(fsize, dtype=np.float32), output.shape[:4])
        to_yshift = np.broadcast_to(
            np.arange(fsize, dtype=np.float32).reshape(fsize, 1), output.shape[:4])
        to_xshift.flags.writeable = True
        to_yshift.flags.writeable = True
        
        x_shift = dtype(to_xshift)
        y_shift = dtype(to_yshift)
        
        
        masked_anchors = np.array(self.masked_anchors)
        
        to_wanchors = np.broadcast_to(np.reshape(
            masked_anchors[:, 0], (1, self.n_anchors, 1, 1)), output.shape[:4])
        to_hanchors = np.broadcast_to(np.reshape(
            masked_anchors[:, 1], (1, self.n_anchors, 1, 1)), output.shape[:4])
        to_wanchors.flags.writeable = True
        to_hanchors.flags.writeable = True
        
        w_anchors = dtype(to_wanchors)
        h_anchors = dtype(to_hanchors)
        
        if labels is None: # inference mode
            anchor_num = np.broadcast_to(
                np.arange(self.n_anchors, dtype=np.float32).reshape(1, self.n_anchors, 1, 1), output.shape[:4])
            anchor_num.flags.writeable = True
            anchor_num = dtype(anchor_num)

            output = torch.cat([output,torch.full((batchsize, self.n_anchors, fsize, fsize,1),
                                fill_value=float(self.layer_no), device=torch.device('cuda'))],dim=4)
            output = torch.cat([output,anchor_num.reshape(batchsize, self.n_anchors, fsize, fsize,1)],dim=4)
            output = torch.cat([output,x_shift.reshape(batchsize, self.n_anchors, fsize, fsize,1)],dim=4)
            output = torch.cat([output,y_shift.reshape(batchsize, self.n_anchors, fsize, fsize,1)],dim=4)
        ...
        if labels is None:  # inference mode
            pred[..., :4] *= self.stride
            return pred.reshape(batchsize, -1, n_ch+4).data
